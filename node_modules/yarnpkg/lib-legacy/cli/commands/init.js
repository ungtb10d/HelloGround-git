...
let run = exports.run = (() => {
  var _ref = (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* (config, reporter, flags, args) {
    const lockfile = new (_wrapper || _load_wrapper()).default();
    const install = new (_install || _load_install()).Install(flags, config, reporter, lockfile);
    const manifests = yield install.getRootManifests();
    ...
    for (const entry of keys) {
      ...
      const answer = (yield reporter.question(question)) || def;
      if (answer) {
    ...
    yield install.saveRootManifests(manifests);
  });
